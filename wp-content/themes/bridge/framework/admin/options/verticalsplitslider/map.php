...
$verticalSplitSliderPage = new QodeAdminPage("_vertical_split_slider", "垂直分割幻灯片", "fa fa-arrows-v");
...
$panel10 = new QodePanel("常规风格","general_style");
	...
	$group1 = new QodeGroup("导航风格","定义导航项目符号风格");
        $panel10->addChild("group1", $group1);

        $row1 = new QodeRow();
        $group1->addChild("row1", $row1);

			$vss_navigation_inactive_color = new QodeField("colorsimple","vss_navigation_inactive_color","","导航颜色","定义导航点颜色");
        $row1->addChild("vss_navigation_inactive_color", $vss_navigation_inactive_color);

			$vss_navigation_inactive_border_color = new QodeField("colorsimple","vss_navigation_inactive_border_color","","导航边框颜色","定义导航点边框颜色");
        $row1->addChild("vss_navigation_inactive_border_color", $vss_navigation_inactive_border_color);

			$vss_navigation_color = new QodeField("colorsimple","vss_navigation_color","","导航活动颜色","定义导航点活动颜色");
        $row1->addChild("vss_navigation_color", $vss_navigation_color);

			$vss_navigation_border_color = new QodeField("colorsimple","vss_navigation_border_color","","导航活动边框颜色","定义导航点活动边框颜色");
        $row1->addChild("vss_navigation_border_color", $vss_navigation_border_color);

    $vss_navigation_size = new QodeField("text","vss_navigation_size","","导航大小 (px)","定义导航点大小", array(), array("col_width" => 1));
        $panel10->addChild("vss_navigation_size", $vss_navigation_size);

    $vss_left_panel_size = new QodeField("text","vss_left_panel_size","","左侧面板大小 (%)","定义左侧面板大小。注意左右面板总和为100.", array(), array("col_width" => 1));
        $panel10->addChild("vss_left_panel_size", $vss_left_panel_size);

    $vss_right_panel_size = new QodeField("text","vss_right_panel_size","","右侧面板大小 (%)","定义右侧面板大小。注意左右面板总和为100.", array(), array("col_width" => 1));
       ...
       qode_add_admin_field(
        ...
        qode_add_admin_field( array(
                ...
                'description'   => esc_html__('选择高级自适应选项将生效的宽度.'),
