...
if ( ! function_exists( 'qode_get_custom_sidebars' ) ) {
	...
	function qode_get_custom_sidebars() {
		$themenametd_custom_sidebars = get_option( 'qode_sidebars' );
		$formatted_array             = array();

		if ( is_array( $themenametd_custom_sidebars ) && count( $themenametd_custom_sidebars ) ) {
			foreach ( $themenametd_custom_sidebars as $custom_sidebar ) {
...
if(!function_exists('qode_send_contact_page_form')) {
	...
	function qode_send_contact_page_form() {
		...
		if(isset($_POST['form_data'])){
			...
			$text = "名字：" . $first_name . " " . $last_name . "\n";
			$text .= "邮箱：" . $data_array['email'] . "\n";
			$text .= "网站：" . $website . "\n";
			$text .= "信息：" . $message;

			$headers = "MIME-Version: 1.0" . "\r\n";
			$headers .= "Content-type:text/plain; charset=utf-8" . "\r\n";
			$headers .= "来自：'".$first_name." ".$last_name."' <".$email_from."> " . "\r\n";
